 ...
 */
export const createBulkNotifications = async (userIds, branchId, title, body, type) => {
  if (!userIds?.length) return
  const payload = { branchId, title, body, type, isRead: false, createdAt: Timestamp.now() }
  await Promise.all(
    userIds.map(userId =>
