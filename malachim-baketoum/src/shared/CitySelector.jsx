...
export default function CitySelector({
  ...
  // ── Render ──────────────────────────────────────────────────────────────
  const inputClass = `w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-gray-200 placeholder-gray-500 focus:outline-none focus:border-orange-500 text-sm`
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      ...
      {/* Dropdown */}
      {open && !loading && (
        <div className="absolute z-50 mt-1 w-full max-h-64 overflow-y-auto bg-gray-900 border border-gray-700 rounded-xl shadow-xl">
          {filtered.length === 0 ? (
              ...
              return (
                <button key={city} type="button"
                  ...
                  className={`w-full text-right px-3 py-2 text-sm transition flex items-center justify-between
                    ${isSelected ? 'bg-orange-500/20 text-orange-200' : 'text-gray-300 hover:bg-gray-800'}`}>
                  <span>{city}</span>
          ...
          {allCities.length > 100 && !query && (
            <div className="px-3 py-2 text-xs text-gray-500 text-center border-t border-gray-800">
