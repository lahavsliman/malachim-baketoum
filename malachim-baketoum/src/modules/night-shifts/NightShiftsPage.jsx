  ...
  const handleConfirm = async () => {
    ...
    try {
      if (selectedShift?.volunteerId === user?.id) {
        ...
        closeModal()
        setSuccessMsg('נרשמת בהצלחה! אתה בהגרלה 🎉')
      }
      await loadShifts()
    } catch (err) {
    ...
    <div className="p-4 sm:p-6 max-w-4xl mx-auto" dir="rtl">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition
                ${activeTab === 'calendar' ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
            >
              לוח שנה
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition
                ${activeTab === 'admin' ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
            >
              ניהול
            </button>
          </div>
      ...
      {isSystemAdmin && !branchId && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-300 font-medium">בחר סניף כדי לצפות בשיבוצי לילה</p>
        </div>
      )}

      {/* Success toast */}
      {successMsg && (
        <div className="mb-4 bg-green-500/15 border border-green-500/30 rounded-xl px-4 py-3 text-green-300 text-center font-medium">
          {successMsg}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className="p-2 rounded-xl bg-gray-800 hover:bg-gray-700 disabled:opacity-30 disabled:cursor-not-allowed text-gray-200 transition text-xl leading-none"
          aria-label="חודש קודם"
        >
          ›
        </button>
        <div className="text-center">
          <h2 className="text-lg font-bold text-gray-200 capitalize">{monthLabel}</h2>
          {isNextMonth && (
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-xl text-sm font-medium transition
              ${isCurrentMonth ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200 border border-gray-700'}`}
          >
            חודש נוכחי
          </button>
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-xl text-sm font-medium transition
              ${isNextMonth ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200 border border-gray-700'}`}
          >
            📅 חודש הבא
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl" dir="rtl">
            <div className="text-center mb-5">
              ...
              <h2 className="text-xl font-bold text-gray-200 mt-3">פרטי מתנדב</h2>
            </div>
            ...
            {viewUserLoading ? (
              ...
              <div className="space-y-2.5">
                <div className="bg-gray-800 rounded-xl px-4 py-3">
                  ...
                  <p className="text-gray-200 font-medium">{viewShift.volunteerName || '—'}</p>
                </div>
                <div className="bg-gray-800 rounded-xl px-4 py-3">
                  <p className="text-xs text-gray-400 mb-0.5">קוד כונן</p>
                  <p className="text-gray-200 font-mono">{viewUser?.volunteerId || '—'}</p>
                </div>
                <div className="bg-gray-800 rounded-xl px-4 py-3">
                  <p className="text-xs text-gray-400 mb-0.5">טלפון</p>
                  <p className="text-gray-200" dir="ltr">{viewUser?.phone || '—'}</p>
                </div>
              </div>
            )}

            <button onClick={() => setViewShift(null)}
              className="mt-5 w-full bg-gray-800 hover:bg-gray-700 text-gray-200 py-2.5 rounded-xl font-medium transition">
              סגור
            </button>
          </div>
        </div>
