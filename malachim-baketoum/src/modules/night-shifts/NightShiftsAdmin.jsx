  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          onClick={exportExcel}
          className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-gray-200 px-4 py-2 rounded-xl font-medium transition border border-gray-700"
        >
          📊 ייצוא Excel
        </button>
      </div>
      ...
      {lotteryHistory.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          <h3 className="font-bold text-gray-200 mb-3">🏆 הגרלות אחרונות</h3>
          <div className="space-y-2">
            {lotteryHistory.map(r => (
              <div key={r.id} className="flex items-center justify-between bg-gray-800/50 px-4 py-2 rounded-xl text-sm">
                <span className="text-gray-300">{r.winnerName}</span>
      ...
      {/* Manual assign */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
        <h3 className="font-bold text-gray-200 mb-3">שיבוץ ידני</h3>
        <div className="flex flex-wrap gap-3">
          <input
            type="date"
            value={assignDate}
            onChange={e => { setAssignDate(e.target.value); setAssignError('') }}
            className="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-gray-200 focus:outline-none focus:border-orange-500"
          />
          <select
            value={assignVolId}
            onChange={e => setAssignVolId(e.target.value)}
            className="bg-gray-800 border border-gray-700 rounded-xl px-3 py-2 text-gray-200 focus:outline-none focus:border-orange-500 flex-1 min-w-32"
          >
