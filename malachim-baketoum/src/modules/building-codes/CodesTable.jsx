...
export default function CodesTable({ codes, onEdit, onRefresh, canEdit, canDelete }) {
  ...
  return (
    <div className="space-y-3">
      {codes.map(code => (
        <div key={code.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 transition hover:border-gray-700">
          {/* Top row: address + actions */}
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="font-bold text-gray-200 text-base">
                🏠 {code.city}, {code.street} {code.buildingNumber}
                {code.entrance && <span className="text-gray-400"> — כניסה {code.entrance}</span>}
              </p>

              {/* Code reveal */}
              <div className="flex items-center gap-2 mt-2">
                <span className="text-sm text-gray-500">🔑 קוד:</span>
                <span
                ...
                >
                  {revealed[code.id] ? '🙈 הסתר' : '👁 הצג קוד'}
                </button>
              </div>

              {/* Entrance */}
              {code.entrance && (
                <p className="text-sm text-gray-400 mt-1">🚪 כניסה: {code.entrance}</p>
              )}

              {/* Notes */}
              {code.notes && (
                <p className="text-sm text-gray-400 mt-1">📝 {code.notes}</p>
              )}

              {/* Meta row */}
              <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-600">
                {code.updatedAt && (
                  <span>🕐 עודכן: {fmtDate(code.updatedAt)}{code.updatedByName ? ` ע״י ${code.updatedByName}` : ''}</span>
                )}
                {(code.changeLog?.length > 0) && (
                  ...
                  >
                    📋 היסטוריה ({code.changeLog.length})
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2 shrink-0">
              {canEdit && (
                <button
                  onClick={() => onEdit(code)}
                  className="text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 px-3 py-1.5 rounded-lg transition border border-gray-700"
                >
                  ✏️ ערוך
                </button>
              )}
          ...
          {expanded[code.id] && code.changeLog?.length > 0 && (
            <div className="mt-3 pt-3 border-t border-gray-800 space-y-2">
              ...
              {[...code.changeLog].reverse().map((entry, i) => (
                <div key={i} className="text-xs bg-gray-800/50 rounded-xl px-3 py-2 space-y-0.5">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-400">{entry.changedByName || entry.changedBy}</span>
