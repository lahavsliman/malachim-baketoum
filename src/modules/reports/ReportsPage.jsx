...
import { Moon, Star, UsersThree, Lock, Globe } from '@phosphor-icons/react'
...
const MonthPicker = ({ value, onChange }) => (
  <input
    ...
    className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-gray-800 text-sm focus:outline-none focus:border-orange-500"
  />
)
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <SectionBox title="לוח חודשי">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
            ...
            const cls = isCovered
              ...
              : isPast ? 'bg-red-500/25 text-red-400' : 'bg-gray-100 text-gray-500'
            return (
        ...
        <div className="flex gap-4 mt-3 text-xs text-gray-500">
          ...
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-100 inline-block" />עתידי</span>
        </div>
      </SectionBox>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-right py-2 px-3 text-gray-500 font-medium text-xs">תאריך</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium text-xs">מתנדב</th>
                </tr>
              </thead>
              <tbody>
                {[...shifts].sort((a, b) => a.date.localeCompare(b.date)).map(s => (
                  <tr key={s.id} className="border-b border-gray-200 hover:bg-gray-100">
                    <td className="py-2 px-3 text-gray-500 text-xs">{fmtShort(s.date)}</td>
                    <td className="py-2 px-3 text-gray-800">{s.volunteerName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <SectionBox title="פירוט לפי שבת">
        {fridayCoverage.length === 0 ? <EmptyState /> : fridayCoverage.map(({ date, shifts: dayShifts }) => {
          ...
          return (
            <div key={date} className="mb-4 pb-4 border-b border-gray-200 last:border-0 last:mb-0 last:pb-0">
                  ...
                  {areas.map(({ name, required }) => {
                    ...
                    return (
                      <div key={name} className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-800">{vols.join(', ') || '—'}</span>
                      </div>
                    )
                  })}
                </div>
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  {['שם','תאריך','מגיעים','אולי','לא מגיעים','לא ענו','% מענה'].map(h => (
                    <th key={h} className="text-right py-2 px-2 text-gray-500 font-medium text-xs whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {tableData.map(e => (
                  <tr key={e.id} className="border-b border-gray-200 hover:bg-gray-100">
                    <td className="py-2 px-2 text-gray-800 font-medium max-w-32 truncate">{e.title}</td>
                    <td className="py-2 px-2 text-gray-500 text-xs whitespace-nowrap">{fmtShort(e.date)}</td>
          ...
          {[['name','שם'],['night','לילה'],['shabbat','שבת'],['active','פעיל']].map(([val, lbl]) => (
            <button key={val} onClick={() => setSortBy(val)}
              className={`px-3 py-1 rounded-xl text-xs font-medium transition
                ${sortBy === val ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500 hover:text-gray-800 border border-gray-200'}`}>
              {lbl}
            </button>
          ))}
        </div>
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  {['שם','תורניות לילה','תורני שבת','סטטוס'].map(h => (
                    <th key={h} className="text-right py-2 px-3 text-gray-500 font-medium text-xs">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {sorted.map(v => (
                  <tr key={v.id} className="border-b border-gray-200 hover:bg-gray-100">
                    <td className="py-2.5 px-3 text-gray-800 font-medium">{v.firstName} {v.lastName}</td>
                    ...
                    <td className="py-2.5 px-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full
                        ${v.active ? 'bg-green-500/15 text-green-400' : 'bg-gray-100 text-gray-500'}`}>
...
const TABS = [
  { Icon: Moon,        label: 'תורניות לילה' },
  { Icon: Star,        label: 'תורני שבת' },
  { Icon: UsersThree,  label: 'ערבי גיבוש' },
  ...
  if (!canAccess) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-64 text-center">
        ...
        <p className="text-gray-500">דף זה מיועד למנהלי סניף בלבד.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto pb-20 lg:pb-0" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">📊 דוחות</h1>
      ...
      {isSystemAdmin && !branchId ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-700 font-medium">בחר סניף כדי לצפות בדוחות</p>
        </div>
      ) : (
        <>
          {/* Tabs */}
          <div className="flex gap-1 bg-white p-1 rounded-2xl border border-gray-200 mb-6 overflow-x-auto">
            {TABS.map((t, i) => (
              <button key={i} onClick={() => setActiveTab(i)}
                ...
                <t.Icon size={15} className="inline ml-1" /> {t.label}
              </button>
            ))}
          </div>
