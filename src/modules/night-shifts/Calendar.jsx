...
export default function Calendar({ year, month, shifts, volunteerId, onDayClick, isAdmin, blockFriday, maxPerNight = 1 }) {
  ...
  const getCellClass = (d) => {
    ...
    if (isPast) return 'bg-gray-100 text-gray-600 cursor-not-allowed border-gray-200'
    if (isBlocked) return 'bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200 opacity-60'
    if (isOwn) return 'bg-yellow-50 text-yellow-700 border-yellow-300 cursor-pointer hover:bg-yellow-100'
    if (isTaken) return 'bg-red-50 text-red-600 border-red-200 cursor-pointer hover:bg-red-100'
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-4">
        ...
        {cells.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              {isBlocked && (
                <Lock size={10} className="mt-0.5 opacity-60 text-gray-500" />
              )}

              {holiday && (
                <span className="text-[8px] leading-tight mt-0.5 max-w-full truncate px-0.5 text-amber-600 opacity-90">
                  {holiday}
                </span>
              )}
            </button>
      ...
      <div className="flex flex-wrap gap-3 mt-4 text-xs text-gray-500">
        ...
        {blockFriday && (
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-gray-200 border border-gray-300 inline-block opacity-60" />
            חסום (שישי)
          </span>
        )}
      </div>
