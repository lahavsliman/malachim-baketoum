...
import { getAllUsers, createUser, updateUser, callUpdateUserEmail, deleteUserDoc } from '../firebase/users'
import { getBranchSettings, updateBranchSettings, deleteBranch } from '../firebase/branches'
...
import {
  Moon, Star, Buildings, Sliders, UsersThree,
  ChartBar, PencilSimple, Trash, Plus, ClipboardText,
  FloppyDisk, Globe,
...
function UserModal({ editUser, branches, onSave, onClose, isNew = false }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-lg max-h-[92vh] overflow-y-auto" dir="rtl">
        <h3 className="font-bold text-gray-800 text-lg mb-5 flex items-center gap-2">
          {isNew ? <Plus size={18} /> : <PencilSimple size={18} />}
          {isNew ? 'הוספת משתמש' : `עריכת ${editUser?.firstName} ${editUser?.lastName}`}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {form.role !== 'branch_head' && form.role !== 'branch_deputy' && (
            <>
              {/* Role types */}
              <div>
                  ...
                  {ROLE_TYPES.map(r => (
                    <button key={r.value} type="button" onClick={() => toggleRole(r.value)}
                      className={`px-3 py-1 rounded-xl text-xs font-medium transition
                        ${form.roleTypes.includes(r.value) ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500 hover:text-gray-800 border border-gray-200'}`}>
                      {r.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <label key={k} className="flex items-center gap-2 cursor-pointer bg-gray-100 px-3 py-2 rounded-xl">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2.5 rounded-xl transition text-sm">ביטול</button>
...
function BranchDetail({ branch, allUsers, branches, onBack, showToast, onDataChange }) {
  ...
  const handleSaveUser = async (form, uid) => {
    ...
    const idChanged = !!(existing && oldId && newId && oldId !== newId)
    if (idChanged) {
      await callUpdateUserEmail(uid, newId)
    }
    ...
    showToast('success', idChanged
      ? 'תעודת הזהות עודכנה בהצלחה'
      : 'המתנדב עודכן')
    setEditUser(null)
    onDataChange()
  }
  ...
  return (
    <div dir="rtl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="text-gray-500 hover:text-gray-800 transition p-1.5 rounded-lg hover:bg-gray-100">← חזרה</button>
        <div>
          <h2 className="text-xl font-black text-gray-900">{branch.name}</h2>
          <p className="text-sm text-gray-500">{branchUsers.length} מתנדבים{head ? ` · ראש סניף: ${head.firstName} ${head.lastName}` : ''}</p>
        </div>
        <span className={`mr-auto text-xs px-2.5 py-1 rounded-full ${branch.isActive !== false ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 text-gray-500'}`}>
          {branch.isActive !== false ? 'פעיל' : 'לא פעיל'}
        </span>
      </div>

      {/* Sub-tabs */}
      <div className="flex gap-1 mb-5 bg-white p-1 rounded-2xl border border-gray-200">
        {([
          ['volunteers', UsersThree, 'מתנדבים'],
          ['codes',      Buildings,  'קודי בניין'],
          ['settings',   Sliders,    'הגדרות'],
        ]).map(([t, TabIcon, l]) => (
          <button key={t} onClick={() => setSettingsTab(t)}
            className={`inline-flex items-center justify-center gap-1.5 flex-1 py-2 px-3 rounded-xl text-sm font-medium transition
              ${settingsTab === t ? 'bg-orange-500 text-white' : 'text-gray-500 hover:text-gray-800'}`}>
            <TabIcon size={14} />
            {l}
          </button>
        ))}
        ...
        <>
          <div className="flex flex-wrap gap-3 mb-4">
            <input placeholder="🔍 חיפוש שם / קוד כונן" value={search} onChange={e => setSearch(e.target.value)}
              className="flex-1 min-w-40 bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-orange-500" />
              ...
              {[['active','פעילים'],['inactive','לא פעילים'],['all','הכל']].map(([v,l]) => (
                <button key={v} onClick={() => setStatusFilter(v)}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition
                    ${statusFilter === v ? 'bg-orange-500 text-white' : 'text-gray-500 hover:text-gray-800'}`}>{l}</button>
              ))}
            </div>
            <button onClick={() => setShowAddDeputy(true)}
              className="bg-gray-100 hover:bg-gray-200 border border-gray-200 text-orange-300 px-4 py-2 rounded-xl text-sm font-medium transition">
              + הוסף סגן
            </button>
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={v.id} className="border-b border-gray-200">
                      ...
                      <td className="py-2.5 px-3 text-gray-700 font-mono text-xs">{v.volunteerId}</td>
                      <td className="py-2.5 px-3 text-gray-500 text-xs" dir="ltr">{v.phone || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-500 text-xs">{ALL_ROLE_LABELS.find(r => r.value === v.role)?.label || v.role || '—'}</td>
                      ...
                      <td className="py-2.5 px-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${v.isActive !== false ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 text-gray-500'}`}>
                          {v.isActive !== false ? 'פעיל' : 'לא פעיל'}
                        </span>
                      </td>
                      <td className="py-2.5 px-3">
                        <button onClick={() => setEditUser(v)}
                          className="inline-flex items-center gap-1 text-xs text-orange-400 hover:text-orange-300 transition"><PencilSimple size={13} /> ערוך</button>
                      </td>
                    </tr>
                  )
                })}
      ...
      {settingsTab === 'codes' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-gray-500 text-sm">{codes.length} קודים</p>
                ...
                <tbody>
                  {codes.map(c => (
                    <tr key={c.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                      <td className="py-2.5 px-3 text-gray-700 text-xs">{c.city || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-800 text-xs">{c.street || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-700 text-xs">{c.buildingNumber || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-700 text-xs">{c.entrance || '—'}</td>
                      <td className="py-2.5 px-3 font-mono text-orange-300 text-xs">{c.code || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-500 text-xs max-w-32 truncate">{c.notes || '—'}</td>
                      <td className="py-2.5 px-3">
                        <button onClick={() => handleDeleteCode(c.id)}
                          className="inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300 transition"><Trash size={13} /> מחק</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          : (
            <div className="space-y-5 max-w-lg">
              <div className="bg-white border border-gray-200 rounded-2xl p-5">
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-gray-700 text-sm">חסום יום שישי</span>
                  </label>
                </div>
              </div>
              <button onClick={handleSaveSettings} disabled={savingSettings}
                ...
                {savingSettings ? 'שומר...' : <><FloppyDisk size={16} className="inline ml-1" /> שמור הגדרות</>}
              </button>
            </div>
          )
      )}
...
function NewBranchWizard({ onClose, onComplete, showToast }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-xl max-h-[92vh] overflow-y-auto" dir="rtl">
        {/* Header */}
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold text-gray-900 text-lg">🪄 אשף יצירת סניף חדש</h3>
          {!submitting && (
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700 transition text-xl leading-none">✕</button>
          )}
        </div>
          ...
          {[1, 2, 3].map(s => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition
                ${step === s ? 'bg-orange-500 text-white' : step > s ? 'bg-orange-500/30 text-orange-300' : 'bg-gray-100 text-gray-500'}`}>
                {step > s ? '✓' : s}
              </div>
              {s < 3 && <div className={`w-8 h-0.5 ${step > s ? 'bg-orange-500/60' : 'bg-gray-100'}`} />}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <h4 className="font-bold text-gray-800 mb-2">פרטי הסניף</h4>
            <div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h4 className="font-bold text-gray-800 mb-2">ראש סניף (חובה)</h4>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-gray-800">סגן ראש סניף (לא חובה)</h4>
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-700">הוסף סגן</span>
              </label>
            </div>
            {includeDeputy
              ? userFields(deputy, setDeputy, 'סגן ראש הסניף')
              : <p className="text-sm text-gray-500 bg-gray-100 rounded-xl px-4 py-6 text-center">דלג על שלב זה — תוכל להוסיף סגן גם בהמשך מתוך עמוד ניהול המשתמשים.</p>
            }
          </div>
        )}
        ...
        <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200">
          {step > 1 && (
            <button type="button" onClick={back} disabled={submitting}
              className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 disabled:opacity-40 text-gray-800 rounded-xl transition text-sm">
              ← חזור
            </button>
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard Icon={Buildings}  label="סניפים פעילים"        value={activeBranches} sub={`מתוך ${branches.length}`} color="text-green-400" />
        <StatCard Icon={UsersThree} label="סה״כ מתנדבים במערכת" value={totalUsers}                                       color="text-orange-400" />
        <StatCard Icon={Moon}       label="שיבוצי לילה החודש"   value={totalNight}                                       color="text-blue-400"   />
        <StatCard Icon={Star}       label="תורני שבת החודש"     value={totalShabbat}                                     color="text-purple-400" />
      </div>

      {/* Per-branch table */}
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-200">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><ClipboardText size={16} /> פילוח לפי סניף</h3>
        </div>
              ...
              {rows.length === 0 ? (
                ...
                return (
                  <tr key={b.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                    <td className="py-2.5 px-3 text-gray-800 font-medium whitespace-nowrap">{b.name}</td>
                    <td className="py-2.5 px-3 text-gray-700 text-center">{bUsers.length}</td>
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-2 min-w-32">
                        ...
                        <span className="text-xs text-gray-500 font-mono w-10 text-left">{coverage}%</span>
                      </div>
                    </td>
                    <td className="py-2.5 px-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap
                        ${b.isActive !== false ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 text-gray-500'}`}>
      ...
      {/* Bar chart */}
      <div className="bg-white border border-gray-200 rounded-2xl p-5">
        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><ChartBar size={16} /> מתנדבים פעילים לפי סניף</h3>
        {chartData.length === 0 ? (
...
export default function SystemAdminPage() {
  ...
  const handleDeleteVolunteer = async () => {
    ...
    try {
      console.log('[SystemAdmin] Deleting user doc:', deleteTarget.id, deleteTarget.firstName, deleteTarget.lastName)
      await deleteUserDoc(deleteTarget.id)
      setUsers(u => u.filter(x => x.id !== deleteTarget.id))
      showToast('success', 'המתנדב נמחק')
    } catch (err) {
      console.error('[SystemAdmin] Delete failed — code:', err?.code, '| message:', err?.message, err)
  ...
  const handleDeleteBranch = async () => {
    ...
    try {
      ...
      const CHUNK = 500
      for (let i = 0; i < branchUsers.length; i += CHUNK) {
        await Promise.all(branchUsers.slice(i, i + CHUNK).map(u => deleteUserDoc(u.id)))
      }
      await deleteBranch(branch.id)
      showToast('success', `הסניף ו-${branchUsers.length} המתנדבים שלו נמחקו`)
      if (selectedBranch?.id === branch.id) setSelectedBranch(null)
      loadData()
    } catch (err) {
      showToast('error', 'שגיאה במחיקת הסניף')
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto" dir="rtl">
      ...
      <h1 className="text-2xl font-black text-gray-900 mb-6 flex items-center gap-2"><Globe size={24} className="text-gray-600" /> ניהול מערכת</h1>

      {/* Top tabs */}
      <div className="flex gap-1 mb-6 bg-white p-1 rounded-2xl border border-gray-200">
        {([
          ['overview', ChartBar,   'סקירה כללית'],
          ['branches', Buildings,  'סניפים'],
          ['users',    UsersThree, 'משתמשים'],
        ]).map(([t, TabIcon, l]) => (
          <button key={t} onClick={() => { setActiveTab(t); setSelectedBranch(null) }}
            className={`inline-flex items-center justify-center gap-1.5 flex-1 py-2.5 px-4 rounded-xl text-sm font-medium transition
              ${activeTab === t ? 'bg-orange-500 text-white' : 'text-gray-500 hover:text-gray-800'}`}>
            <TabIcon size={15} />
            {l}
          </button>
        ))}
        ...
        <>
          ...
          {activeTab === 'branches' && !selectedBranch && (
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <p className="text-gray-500 text-sm">{branches.length} סניפים</p>
                ...
                {branches.map(b => {
                  ...
                  return (
                    <div key={b.id}
                      ...
                      className="bg-white border border-gray-200 hover:border-orange-500/40 rounded-2xl p-5 cursor-pointer transition-all group">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <h3 className="font-bold text-gray-800 text-lg group-hover:text-orange-300 transition">{b.name}</h3>
                          {b.city && <p className="text-gray-500 text-sm">{b.city}</p>}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`text-xs px-2 py-1 rounded-full ${b.isActive !== false ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 text-gray-500'}`}>
                            {b.isActive !== false ? 'פעיל' : 'לא פעיל'}
                          </span>
                          <button
                          ...
                          <button
                            onClick={e => { e.stopPropagation(); setDeleteBranchTarget(b) }}
                            className="text-xs text-red-500 hover:text-red-400 p-1 rounded-lg hover:bg-red-50 transition"
                          >
                            <Trash size={13} />
                          </button>
                        </div>
                      ...
                      <div className="flex gap-4 text-sm text-gray-500">
                        <span className="inline-flex items-center gap-1"><UsersThree size={14} /> {active} מתנדבים פעילים ({bUsers.length} סך הכל)</span>
                      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                <input
                  ...
                  className="flex-1 min-w-52 bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-orange-500"
                />
                <select value={branchFilter} onChange={e => setBranchFilter(e.target.value)}
                  className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-700 focus:outline-none focus:border-orange-500">
                ...
                <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
                  ...
                  {ALL_ROLE_LABELS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                </select>
                    ...
                    {filteredUsers.length === 0 ? (
                      ...
                      return (
                        <tr key={u.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                          <td className="py-2.5 px-3 text-gray-800 font-medium whitespace-nowrap">{u.firstName} {u.lastName}</td>
                          <td className="py-2.5 px-3 text-gray-500 text-xs font-mono">{u.idNumber || '—'}</td>
                          <td className="py-2.5 px-3 text-gray-500 text-xs font-mono">{u.volunteerId || '—'}</td>
                          <td className="py-2.5 px-3 text-gray-500 text-xs">{branches.find(b => b.id === u.branchId)?.name || '—'}</td>
                          <td className="py-2.5 px-3">
                            <span className="text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">
                              {ALL_ROLE_LABELS.find(r => r.value === u.role)?.label || u.role || '—'}
                            </span>
                          </td>
                          <td className="py-2.5 px-3">
                            <div className="flex flex-wrap gap-1">
                          ...
                          <td className="py-2.5 px-3">
                            <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap
                              ${u.isActive !== false ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 text-gray-500'}`}>
                          ...
                          <td className="py-2.5 px-3">
                            <div className="flex items-center gap-3">
                              ...
                              <button onClick={() => setDeleteTarget(u)}
                                className="inline-flex items-center gap-1 text-xs text-red-500 hover:text-red-400 transition"><Trash size={13} /> מחק</button>
                            </div>
                          </td>
                        </tr>
                      )
      ...
      {/* Delete confirmation modal — volunteer */}
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
      ...
      {/* Delete confirmation modal — branch */}
      {deleteBranchTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
          onClick={e => e.target === e.currentTarget && setDeleteBranchTarget(null)}
          dir="rtl">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="text-center mb-5">
              <Trash size={40} className="mx-auto mb-3 text-red-400" />
              <h3 className="font-bold text-gray-900 text-lg">מחיקת סניף</h3>
              <p className="text-gray-500 text-sm mt-2">
                האם למחוק את הסניף{' '}
                <span className="font-semibold text-gray-900">{deleteBranchTarget.name}</span>{' '}
                וכל המתנדבים שלו? פעולה זו אינה ניתנת לביטול.
              </p>
              <p className="text-red-500 text-xs mt-2 font-medium">
                {users.filter(u => u.branchId === deleteBranchTarget.id).length} מתנדבים ימחקו לצמיתות
              </p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setDeleteBranchTarget(null)}
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2.5 rounded-xl transition text-sm font-medium">
                ביטול
              </button>
